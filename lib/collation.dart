...
enum CollationEncoding {
  ...
  final String code;
  const CollationEncoding(this.code);
}

const Map<int, String> codepageByLanguageId = {
  // Arabic_*
  0x0401: 'CP1256',
...
const Map<int, String> codepageBySortId = {
...
class Collation {
  ...
  factory Collation.fromBuffer(Buffer buffer, {int offset = 0}) {
    ...
    var flags = (buffer[offset + 3] & 0x0F) << 4;
    flags |= (buffer[offset + 2] & 0xF0) >>> 4;

    var version = (buffer[offset + 3] & 0xF0) >>> 4;

    var sortId = buffer[offset + 4];
