...
final EPOCH_DATE = DateTime(1, 1).toLocal();
// LocalDate.ofYearDay(1, 1);
final NULL_LENGTH = Buffer.from([0x00]);

class DateTime2 extends DataType {
  @override
  String declaration(Parameter parameter) {
    return "datetime2(' ${resolveScale(parameter)} ')";
  }
  ...
  Stream<Buffer> generateParameterData(
    ...
    DateTime date;
    if (options.useUTC) {
      date = DateTime.utc(value.year, value.month + 1, value.day).toLocal();
    } else {
  ...
  static int get refID => 0x2A;

  @override
  String get name => 'DateTime2';
  ...
  @override
  String get type => 'DATETIME2N';

  @override
  validate(dynamic value, Collation? collation) {
    if (value == null) {
      return null;
    }
